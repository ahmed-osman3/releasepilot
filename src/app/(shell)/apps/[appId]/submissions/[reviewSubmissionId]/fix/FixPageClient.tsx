...
export default function FixPageClient({
  ...
  const [editedValues, setEditedValues] = useState<Record<string, SuggestedLocaleEdits>>({})
  ...
  const handleApply = async () => {
    ...
    try {
      for (const [localeId, updates] of Object.entries(editedValues)) {
        const result = await applyVersionMetadataServer({
          connectedAppId,
          versionId,
          localeId,
          updates,
                ...
                {hasEdits && (
                  <div className="animate-fade-in-up space-y-4">
                    <div className="flex items-center justify-between">
                      <span className="font-display text-sm font-medium text-foreground">Suggested Edits</span>
                      <span className="text-[11px] text-muted-foreground">Review and edit before applying</span>
                    </div>
                    {Object.entries(editedValues).map(([localeId, edits]) => {
                      ...
                      return (
                        <LocaleEditForm
                          ...
                          edits={edits}
                          onChange={(field, value) => updateEdit(localeId, field, value)}
                        />
                      )
                    })}
  ...
  return (
    <div className="rounded-lg border border-border/30 bg-background/20 p-4 space-y-3">
      ...
      {edits.promotionalText !== undefined && (
        <div className="metadata-field space-y-1">
          <Label className="text-[11px] text-muted-foreground">Promotional Text</Label>
          <Textarea
      ...
      {edits.whatsNew !== undefined && (
        <div className="metadata-field space-y-1">
          <Label className="text-[11px] text-muted-foreground">What&apos;s New</Label>
          <Textarea
